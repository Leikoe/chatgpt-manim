...
class ChatGPTVideo(VoiceoverScene):
    def construct(self):
        ...
        self.play(FadeOut(image), FadeOut(chatgpt_label))
        ...
        # Instruct Finetuning
        with self.voiceover(text="To take the model from being able to complete text to being a useful assistant, we train the model on question answer pairs.") as tracker:
...
"""
        ...
        with self.voiceover(text="We end up with a so called Intruct or Chat model which is able to Answer questions and follow instruction.") as tracker:
            ...
            self.play(Create(model), Write(model_label))
        self.play(FadeOut(model), FadeOut(model_label))
